...
This module connects to the [PhyPhox](https://phyphox.org/) app on a smartphone and produces outputs with the values measured by its sensors. The app is free of charge, and the QualiaSense plugin is not affiliated with PhyPhox.
...
- A smartphone
    - with sensors
    - with the PhyPhox app installed : https://phyphox.org/download/
...
- PhyPhox app settings: configure the sensor type and the `ip:port` of the smartphone.
- Sensor limit settings: allows you to adjust the min and max values retrieved from the sensor for each axis. If a value exceeds a limit, it will be clamped to that limit. Each sensor type has its own default limit values.
- Output voltage mode: unipolar/bipolar. Sets the module to output either a `0V -> +10V` or a `-5V -> +5V` voltage signal.
